...
int main(int, char**)
{
    ...
    for (auto rr : r) { std::cout << rr.first << ": " << rr.second << std::endl; }
    auto i = vsa::village::EntitiesRegistry::get_instance().get_item();
    for (auto ir : i) { std::cout << ir.first << ": " << ir.second << std::endl; }
